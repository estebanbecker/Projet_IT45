...
public class SESSAD {
                ...
                for(int k = 0; k < solution[i][j].size(); k++) {
                    for(int l = j+1; l < solution[i].length; l++) {
                        for(int m = k+1; m < solution[i][l].size(); m++) {
                            if(solution[i][j].get(k) == solution[i][l].get(m) && solution[i][j].get(k) != -1 && solution[i][l].get(k) >= this.center_name.length) {
    ...
    public ArrayList<Integer>[][] deepcopy(ArrayList<Integer>[][] originalArray) {
        ...
        // iterate over the elements of the original array and create a new ArrayList for each element
